...
Primary navigation: **Home**, **Discover**, **Library**, with a mini-player above the bar. Home is artwork-first (hero + horizontal shelves). Discover, track detail, and Library rows show availability badges (stream, download, offline, unavailable, attribution). Now playing streams from the provider URL, shows the queue, and opens a license sheet.
