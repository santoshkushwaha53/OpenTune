...
3. Optional: `AUDIUS_API_KEY`, `JAMENDO_CLIENT_ID`, `YOUTUBE_API_KEY`, `INTERNET_ARCHIVE_ENABLED`, `OPERATOR_TOKEN`, `CORS_ORIGIN`.
4. Start the stack:
...
To persist open-catalog singers for Discover search (metadata only), run an operator catalog sync after Jamendo/Audius are enabled:

```bash
curl -X POST "$API_BASE/api/v1/providers/jamendo/sync" \
  -H "x-opentune-operator: $OPERATOR_TOKEN" \
  -H "content-type: application/json" \
  -d '{"query":"*"}'
```

Repeat for `audius`, `youtube`, and `archive` if those providers are enabled. YouTube sync stores video metadata only. Playback uses YouTube’s official player on the device. It does not import MP3s or commercial download catalogs.
