/**
 * Audius `tracks/search` is a text query, not Jamendo tags.
 * Scene chips like Bollywood need extra licensed-catalog terms.
 */
const AUDIUS_QUERY_ALIASES: Record<string, string[]> = {
...
export function audiusSearchQueries(query: string): string[] {
  const key = query.trim().toLowerCase().replace(/\s+/g, " ");
  if (!key || key === "*") {
    return [];
  }
  return [...new Set([key, ...(AUDIUS_QUERY_ALIASES[key] ?? [])])];
}
